      ...
      return (
          <Layout>
              <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
                  ...
                  <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">No active orders</h1>
                  <p className="text-gray-500 mb-6">You haven't placed any orders yet.</p>
                  <Link to="/" className="px-6 py-3 bg-primary text-white rounded-full font-bold">Start Shopping</Link>
              </div>
          </Layout>
      )
  }
    ...
    <Layout>
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-10 py-8">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
                <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white mb-2">Track Your Order</h1>
                <p className="text-gray-500 dark:text-gray-400 text-lg">Keep up with your latest kicks.</p>
            </div>
            <button className="flex items-center gap-2 text-sm font-bold text-primary hover:underline">
                <span className="material-symbols-outlined text-[18px]">help</span> Need Help?
            </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-8 flex flex-col gap-8">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/10 pb-4">
                    <h2 className="text-2xl font-bold tracking-tight">Current Order <span className="text-primary">#{latestOrder.id}</span></h2>
                    <span className="hidden sm:inline-flex bg-primary/10 text-primary text-sm font-bold px-3 py-1 rounded-full items-center gap-1">
                        <span className="w-2 h-2 rounded-full bg-primary animate-pulse"></span> {latestOrder.status}
                    </span>
                </div>
                ...
                <div className="bg-white dark:bg-surface-dark rounded-xl p-6 shadow-sm border border-gray-100 dark:border-white/5">
                    <div className="flex flex-col md:flex-row gap-8">
                         ...
                         <div className="flex-1 flex flex-col justify-center">
                            <div className="mb-6">
                                <p className="text-primary font-bold text-sm tracking-widest uppercase mb-1">On its way</p>
                                <h3 className="text-3xl font-black text-slate-900 dark:text-white leading-tight mb-2">{mainItem.name}</h3>
                                <p className="text-gray-500 dark:text-gray-400 text-base">Size: {mainItem.selectedSize} • Color: {mainItem.selectedColor} • Qty: {mainItem.quantity}</p>
                                {latestOrder.items.length > 1 && (
                                    <p className="text-sm font-bold text-gray-500 mt-1">+ {latestOrder.items.length - 1} other items</p>
                                )}
                            </div>
                            <div className="flex flex-col sm:flex-row gap-4 sm:items-end justify-between mt-auto">
                                ...
                                <div className="flex gap-3">
                                    <button className="flex-1 sm:flex-none h-10 px-6 bg-white border border-gray-200 dark:border-white/20 dark:bg-transparent dark:text-white rounded-full text-sm font-bold hover:bg-gray-50 dark:hover:bg-white/10 transition-colors">View Invoice</button>
                                </div>
                            </div>
                         </div>
                    </div>
                    ...
                    <div className="mt-10 pt-8 border-t border-gray-100 dark:border-white/10">
                        <div className="relative px-2">
                             ...
                             <div className="relative z-10 flex justify-between w-full">
                                 {[
                                     {icon: 'check', label: 'Order Placed', date: latestOrder.date, active: true},
                                     {icon: 'local_shipping', label: 'Shipped', date: 'Pending', active: true},
                                     {icon: 'location_on', label: 'Out for Delivery', date: 'Pending', active: true, current: true},
                                     {icon: 'inventory_2', label: 'Delivered', date: 'Pending', active: false}
                                 ].map((step, i) => (
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
                <div className="bg-promotion rounded-xl p-6 relative overflow-hidden group">
                     <div className="relative z-10">
                         <span className="inline-block py-1 px-3 bg-black text-white text-xs font-bold rounded-full mb-3 uppercase tracking-wider">New Arrival</span>
                         <h3 className="text-2xl font-black text-black leading-tight mb-2">Match Your<br/>New Kicks</h3>
                         <p className="text-black/80 font-medium text-sm mb-4 max-w-[160px]">Check out our premium socks collection.</p>
                         <button className="bg-black text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-gray-900 transition-colors">Shop Accessories</button>
                     </div>
                ...
                <div className="bg-white dark:bg-surface-dark rounded-xl border border-gray-100 dark:border-white/5 overflow-hidden flex-1">
                    <div className="p-5 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
                         <h3 className="font-bold text-lg text-slate-900 dark:text-white">Order History</h3>
                             ...
                             <div key={i} className="p-4 flex gap-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                                 ...
                                 <div className="flex-1 min-w-0">
                                     <div className="flex justify-between items-start mb-1">
                                         ...
                                         <span className="text-xs font-bold text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400 px-2 py-0.5 rounded-full whitespace-nowrap">{item.status}</span>
                                     </div>
                                     <p className="text-xs text-gray-500 mb-2">{item.date}</p>
                                     <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                         <button className="text-xs font-bold text-primary hover:text-primary-hover">Buy Again</button>
                                         <button className="text-xs font-medium text-gray-500 hover:text-slate-900 dark:hover:text-white">View Receipt</button>
                                     </div>
                                 </div>
                             </div>
                         ))}
